...
export const getAllDistricts  = async (req, res, next) => {
    try {
        ...
        res.status(200).json({message: `found all Disrtict`, data: getAllDistrict })
    }catch(error) {
        console.log(error);
        next(error)
    }
...
export const getOneDistricts = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ message: `found ID ${req.params.id} from Disrtict`, data: getOneDistrict })
    }catch(error) {
        console.log(error);
        next(error)
    }
