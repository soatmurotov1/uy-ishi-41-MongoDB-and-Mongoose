...
export const getOnewater  = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ message: `found ID ${req.params.id} from water`, data: getAllwater })
    }catch(error) {
        console.log(error);
        next(error)
    }
...
export const deletewater  = async(req, res, next) => {
    try {
        const deletewater  = await water_productsModel.findByIdAndDelete(req.params.id)
        if(!deletewater) {
            return res.status(404).json({message: `not found ID ${req.params.id} from water`})
        }
