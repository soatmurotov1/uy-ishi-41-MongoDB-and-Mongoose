...
export const getAllOrderItem  = async (req, res, next) => {
    try {
        ...
        res.status(200).json({message: `found all order_item`, data: getAllOrder_item})
    }catch(error) {
        console.log(error);
        next(error)
    }
...
export const getOneOrderItem  = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ message: `found ID ${req.params.id} from order_item`, data: getOneOrderItem })
    }catch(error) {
        console.log(error);
        next(error)
    }
...
export const deleteOrderItem  = async(req, res, next) => {
    try {
        const deleteOrder_item  = await order_itemModel.findByIdAndDelete(req.params.id)
        if(!deleteOrder_item) {
            return res.status(404).json({message: `not found ID ${req.params.id} from order_item`})
        }
